# Financial Analysis (CrewAI) - `POST /analyze/financial-report`

## Vue d'ensemble

Analyse financière complète avec équipe de 6 agents CrewAI spécialisés
...
```
Client → POST /analyze/financial-report → API FastAPI → Services spécialisés → Réponse
```
...
| Fichier | Rôle |
|---------|------|
| `api/main.py (243-294)` | Implémentation |
| `agents/financial_crew.py` | Implémentation |
| `api/rag_manager.py` | Implémentation |
...
curl http://localhost:8000POST /analyze/financial-report
```
...
response = requests.get("http://localhost:8000POST /analyze/financial-report")
print(response.json())
```
...
curl -X POST http://localhost:8000POST /analyze/financial-report \
...
curl http://localhost:8000POST /analyze/financial-report/invalid
```
