# Portfolio Building (CrewAI) - `POST /build-portfolio`

## Vue d'ensemble

Construction d'un portefeuille PEA optimal de zéro selon budget et profil
...
```
Client → POST /build-portfolio → API FastAPI → Services spécialisés → Réponse
```
...
| Fichier | Rôle |
|---------|------|
| `api/main.py (297-364)` | Implémentation |
| `agents/portfolio_builder_crew.py` | Implémentation |
...
curl http://localhost:8000POST /build-portfolio
```
...
response = requests.get("http://localhost:8000POST /build-portfolio")
print(response.json())
```
...
curl -X POST http://localhost:8000POST /build-portfolio \
...
curl http://localhost:8000POST /build-portfolio/invalid
```
