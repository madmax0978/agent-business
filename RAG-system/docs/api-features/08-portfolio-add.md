# Portfolio Add Position - `POST /portfolio/add`

## Vue d'ensemble

Ajoute ou renforce une position avec calcul automatique du PRU
...
```
Client → POST /portfolio/add → API FastAPI → Services spécialisés → Réponse
```
...
| Fichier | Rôle |
|---------|------|
| `api/main.py (371-386)` | Implémentation |
...
curl http://localhost:8000POST /portfolio/add
```
...
response = requests.get("http://localhost:8000POST /portfolio/add")
print(response.json())
```
...
curl -X POST http://localhost:8000POST /portfolio/add \
...
curl http://localhost:8000POST /portfolio/add/invalid
```
