# Portfolio Sell Position - `POST /portfolio/sell`

## Vue d'ensemble

Vend une position (partielle ou totale) avec calcul de plus-value
...
```
Client → POST /portfolio/sell → API FastAPI → Services spécialisés → Réponse
```
...
| Fichier | Rôle |
|---------|------|
| `api/main.py (389-398)` | Implémentation |
...
curl http://localhost:8000POST /portfolio/sell
```
...
response = requests.get("http://localhost:8000POST /portfolio/sell")
print(response.json())
```
...
curl -X POST http://localhost:8000POST /portfolio/sell \
...
curl http://localhost:8000POST /portfolio/sell/invalid
```
