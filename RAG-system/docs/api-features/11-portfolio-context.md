# Portfolio Context - `GET /portfolio/context`

## Vue d'ensemble

Génère un contexte formaté pour les agents IA
...
```
Client → GET /portfolio/context → API FastAPI → Services spécialisés → Réponse
```
...
| Fichier | Rôle |
|---------|------|
| `api/main.py (410-415)` | Implémentation |
...
curl http://localhost:8000GET /portfolio/context
```
...
response = requests.get("http://localhost:8000GET /portfolio/context")
print(response.json())
```
...
curl -X POST http://localhost:8000GET /portfolio/context \
...
curl http://localhost:8000GET /portfolio/context/invalid
```
