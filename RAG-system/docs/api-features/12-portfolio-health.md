# Portfolio Health Score - `GET /portfolio/health`

## Vue d'ensemble

Calcule un score de santé 0-100 basé sur diversification et performance
...
```
Client → GET /portfolio/health → API FastAPI → Services spécialisés → Réponse
```
...
| Fichier | Rôle |
|---------|------|
| `api/main.py (418-423)` | Implémentation |
...
curl http://localhost:8000GET /portfolio/health
```
...
response = requests.get("http://localhost:8000GET /portfolio/health")
print(response.json())
```
...
curl -X POST http://localhost:8000GET /portfolio/health \
...
curl http://localhost:8000GET /portfolio/health/invalid
```
