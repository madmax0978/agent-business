# Portfolio Rebalance - `GET /portfolio/rebalance`

## Vue d'ensemble

Recommandations de rééquilibrage si déséquilibre détecté
...
```
Client → GET /portfolio/rebalance → API FastAPI → Services spécialisés → Réponse
```
...
| Fichier | Rôle |
|---------|------|
| `api/main.py (426-431)` | Implémentation |
...
curl http://localhost:8000GET /portfolio/rebalance
```
...
response = requests.get("http://localhost:8000GET /portfolio/rebalance")
print(response.json())
```
...
curl -X POST http://localhost:8000GET /portfolio/rebalance \
...
curl http://localhost:8000GET /portfolio/rebalance/invalid
```
