# Position Details - `GET /portfolio/position/{ticker}`

## Vue d'ensemble

Détails complets d'une position spécifique avec historique
...
```
Client → GET /portfolio/position/{ticker} → API FastAPI → Services spécialisés → Réponse
```
...
| Fichier | Rôle |
|---------|------|
| `api/main.py (434-439)` | Implémentation |
| `services/portfolio_manager.py` | Implémentation |
...
curl http://localhost:8000GET /portfolio/position/{ticker}
```
...
response = requests.get("http://localhost:8000GET /portfolio/position/{ticker}")
print(response.json())
```
...
curl -X POST http://localhost:8000GET /portfolio/position/{ticker} \
...
curl http://localhost:8000GET /portfolio/position/{ticker}/invalid
```
