# Market Stock Info - `GET /market/stock/{ticker}`

## Vue d'ensemble

Informations de marché via Yahoo Finance (gratuit, temps réel)
...
```
Client → GET /market/stock/{ticker} → API FastAPI → Services spécialisés → Réponse
```
...
| Fichier | Rôle |
|---------|------|
| `api/main.py (446-455)` | Implémentation |
...
curl http://localhost:8000GET /market/stock/{ticker}
```
...
response = requests.get("http://localhost:8000GET /market/stock/{ticker}")
print(response.json())
```
...
curl -X POST http://localhost:8000GET /market/stock/{ticker} \
...
curl http://localhost:8000GET /market/stock/{ticker}/invalid
```
