# Market History - `GET /market/history/{ticker}`

## Vue d'ensemble

Historique des cours (1mo à 10y) avec paramètres configurables
...
```
Client → GET /market/history/{ticker} → API FastAPI → Services spécialisés → Réponse
```
...
| Fichier | Rôle |
|---------|------|
| `api/main.py (458-485)` | Implémentation |
...
curl http://localhost:8000GET /market/history/{ticker}
```
...
response = requests.get("http://localhost:8000GET /market/history/{ticker}")
print(response.json())
```
...
curl -X POST http://localhost:8000GET /market/history/{ticker} \
...
curl http://localhost:8000GET /market/history/{ticker}/invalid
```
