# News Analysis - `GET /analysis/news/{company_name}`

## Vue d'ensemble

Actualités récentes d'une entreprise (NewsAPI + SerpAPI + fallback)
...
```
Client → GET /analysis/news/{company_name} → API FastAPI → Services spécialisés → Réponse
```
...
| Fichier | Rôle |
|---------|------|
| `api/main.py (492-497)` | Implémentation |
...
curl http://localhost:8000GET /analysis/news/{company_name}
```
...
response = requests.get("http://localhost:8000GET /analysis/news/{company_name}")
print(response.json())
```
...
curl -X POST http://localhost:8000GET /analysis/news/{company_name} \
...
curl http://localhost:8000GET /analysis/news/{company_name}/invalid
```
