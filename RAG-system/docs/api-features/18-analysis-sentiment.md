# Sentiment Analysis - `GET /analysis/sentiment/{company_name}`

## Vue d'ensemble

Analyse de sentiment avec IA (Claude/GPT-4) des actualités
...
```
Client → GET /analysis/sentiment/{company_name} → API FastAPI → Services spécialisés → Réponse
```
...
| Fichier | Rôle |
|---------|------|
| `api/main.py (500-509)` | Implémentation |
| `services/sentiment_analyzer.py` | Implémentation |
| `services/news_aggregator.py` | Implémentation |
...
curl http://localhost:8000GET /analysis/sentiment/{company_name}
```
...
response = requests.get("http://localhost:8000GET /analysis/sentiment/{company_name}")
print(response.json())
```
...
curl -X POST http://localhost:8000GET /analysis/sentiment/{company_name} \
...
curl http://localhost:8000GET /analysis/sentiment/{company_name}/invalid
```
